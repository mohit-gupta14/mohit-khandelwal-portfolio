...
export const ExperienceSection: React.FC = () => {
  ...
  return (
    <section id="experience" className="py-16 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
      ...
      <div className="p-6 rounded-3xl bg-slate-900/90 border border-white/10 shadow-mobile-card mb-8">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-4 border-b border-white/10">
          ...
          <div className="flex items-center gap-3">
            <span className="px-3.5 py-1.5 rounded-xl bg-white/[0.04] border border-white/10 text-xs font-mono text-slate-300">
              8 Major Engagements
            </span>
            <span className="px-3.5 py-1.5 rounded-xl bg-emerald-500/15 border border-emerald-500/30 text-xs font-mono text-emerald-300 font-bold">
              Active Senior Role
            </span>
          </div>
        </div>
      ...
      <div className="relative pl-6 sm:pl-8 border-l-2 border-rn-cyan/30 space-y-6">
        ...
        {experience.engagements.map((eng, idx) => {
          ...
          return (
            ...
            >
              ...
              <div className="absolute -left-[31px] sm:-left-[39px] top-6 w-4 h-4 rounded-full bg-slate-900 border-2 border-rn-cyan flex items-center justify-center shadow-rn-glow-sm">
              ...
              <div
                ...
                className={`cursor-pointer p-5 sm:p-6 rounded-3xl bg-slate-900/90 border transition-all duration-200 ${
                  isExpanded
                    ? 'border-rn-cyan/40 shadow-xl bg-slate-900'
                    : 'border-white/10 hover:border-white/20 hover:bg-slate-800/80 shadow-mobile-card'
                }`}
              >
                {/* Header info */}
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-3">
                  <div>
                    <div className="flex items-center gap-2">
                      <h4 className="text-base sm:text-lg font-bold text-white group-hover:text-rn-cyan transition-colors">
                        {eng.client}
                      </h4>
                      <span className="text-[10px] font-mono px-2 py-0.5 rounded-full bg-rn-cyan/15 text-rn-cyan border border-rn-cyan/30">
                        {eng.domain}
                      </span>
                    </div>
                ...
                <div className="flex flex-wrap gap-2 pt-1 pb-2">
                  {eng.metrics.map((metric, mIdx) => (
                    <span
                      key={mIdx}
                      className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-emerald-500/10 border border-emerald-500/20 text-[11px] font-medium text-emerald-300"
                    >
                ...
                <AnimatePresence>
                  {isExpanded && (
                    <motion.div
                      ...
                      className="space-y-4 pt-3 border-t border-white/10 mt-2"
                    >
                      <div className="space-y-2 text-xs text-slate-300">
                        {eng.bullets.map((bullet, bIdx) => (
                          <div key={bIdx} className="flex items-start gap-2.5 leading-relaxed">
                            <CheckCircle2 className="w-4 h-4 text-rn-cyan flex-shrink-0 mt-0.5" />
                            <span>{bullet}</span>
                          </div>
                        ))}
                      </div>
                      ...
                      <div className="pt-2">
                        ...
                        <div className="flex flex-wrap gap-1.5">
                          {eng.technologies.map((tech, tIdx) => (
                            <span
                              key={tIdx}
                              className="px-2.5 py-1 rounded-lg bg-white/[0.05] border border-white/10 text-[11px] font-mono text-slate-300"
                            >
                              {tech}
                            </span>
                          ))}
